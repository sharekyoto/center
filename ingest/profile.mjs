/* ============================================================ profile.mjs ===
   ...
   受け取る符（すべて任意・順不同・一行に混ぜてもよい）
     観測員 ナツ                          いちばんやさしい名乗り。報告の末尾に署名として一行
                                          （観測員：ナツ ／ 観測員「ナツ」／ #観測員 ナツ も同じ）
     #呼称 ///ナツ                        旧表記。引き続き受ける
     #所見 ///夜の水路ばかり撮っています
     #区域 ///下京区
     #WIKIDOT ///natsu
...
/* 署名の一行。「観測員 ナツ」。
   観測員証の名乗り「#観測員0042 三語」とは、観測員の直後が数字かどうかで分ける。
   **行まるごとが署名の形のときだけ拾う。** 「観測員が二人いた。」のような本文は、
   区切り（空白・：・「）が無いので拾わない。名前に空白や句点が入っていても拾わない。
   X ではハッシュタグが空白で切れるので、#観測員ナツ（空白なし）も #観測員 ナツ（空白あり）も受ける。
   # が付いているときだけ区切りを省いてよい（#観測員が… という本文はまず無い）。
   名前のあとに同じ行でタグやメンションが続いてもよい（観測員 ナツ #観測センター）。
   その場合、行は落とすがタグは本文に返す（座標や現象のタグを失わないため）。
   取りこぼした人は送り直せる。本文を名前として吸い込むよりずっとよい。 */
const SIGN_RE = /(^|\n)[ 　]*(?:[—―－\-]+[ 　]*)?(?:[#＃]観測員[ 　:：「『]*|観測員(?:[ 　]+|[ 　]*[：:][ 　]*|[ 　]*[「『][ 　]*))([^\s。、，．！？!?#＃@＠「」『』0-9０-９][^\s。、，．！？!?#＃@＠「」『』]{0,19})[ 　]*[」』]?((?:[ 　]+[#＃@＠][^\s]+)*)[ 　]*(?=\n|$)/;
...
export function readProfile(text) {
  ...
  /* 署名の一行を先に抜く。行ごと落とすので、盤にもフィルム片にも名前は載らない */
  const sg = SIGN_RE.exec(rest);
  if (sg) {
    const v = clean(sg[2], CAP.name);
    if (v) fields.name = v;
    const tags = (sg[3] || '').trim();
    rest = rest.slice(0, sg.index) + sg[1] + tags + rest.slice(sg.index + sg[0].length);
  }
